...
# nginx官方下载较慢，可以采用下载工具下载后放到本地 https://www.nginx.net/downloads
# 
RELEASE_URL="http://nginx.org/download/nginx-${NGINX_VERSION}.tar.gz"

# LOCAL_URL="http://docker.lan.yunnan.ws/nginx"

# # 检测本地服务器上面是否存在指定的文件，如果不存在，则使用远程的
# check_http $RELEASE_URL
# if [[ $status_code != 200 ]]; then
#   echo "本地服务器上不存在文件 nginx-${NGINX_VERSION}.tar.xz ，\n使用远程文件：https://www.nginx.net/distributions/nginx-${NGINX_VERSION}.tar.xz ！"
#   RELEASE_URL="http://nginx.org/download/nginx-${NGINX_VERSION}.tar.gz"
#   check_http $RELEASE_URL
...
# 从版本号（8.2.0）获取主版本 如： 8
M_VER=$(echo ${NGINX_VERSION}|cut -d'.' -f1)
# 获取中间版本号 如： 8.1
MS_VER=$(echo ${NGINX_VERSION}|cut -d'.' -f1 -f2)
...
if [ "$DNGINX_VER" ];then
    ...
    # docker push tekintian/alpine-nginx:${M_VER}${TAG_EX}
    # docker push tekintian/alpine-nginx:${TAG}
    # docker rmi tekintian/alpine-nginx:${M_VER}${TAG_EX}
