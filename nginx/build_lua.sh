...
# build_lua.sh -v Nginx版本 -a ALPINE版本 -s 文件sha256
...
# 
# nginx下载地址 https://nginx.org/download/nginx-1.26.2.tar.gz
# 
...
if [[ "$DNGINX_VER" =~ "${MS_VER}" ]];then
    ...
    docker tag tekintian/alpine-nginx:${TAG} tekintian/alpine-nginx:${MS_VER}_lua
    docker tag tekintian/alpine-nginx:${TAG} registry.cn-hangzhou.aliyuncs.com/alpine-docker/nginx:${MS_VER}_lua
    docker tag tekintian/alpine-nginx:${TAG} registry.cn-hangzhou.aliyuncs.com/alpine-docker/nginx:${DNGINX_VER}${TAG_EX}
    ...
    docker push tekintian/alpine-nginx:${MS_VER}_lua

    docker push registry.cn-hangzhou.aliyuncs.com/alpine-docker/nginx:${MS_VER}_lua
    ...
    docker push registry.cn-hangzhou.aliyuncs.com/alpine-docker/nginx:${DNGINX_VER}${TAG_EX}
